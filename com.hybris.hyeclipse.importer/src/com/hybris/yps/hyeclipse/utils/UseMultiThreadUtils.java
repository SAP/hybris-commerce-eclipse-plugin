...
public class UseMultiThreadUtils {
	...
	public static void useMultiThread(File platformHome) {
		...
		if (Files.exists(serverxml) && Files.isRegularFile(serverxml)) {
			try {
				...
				throw new IllegalStateException(
						String.format("Failed to access the server.xml file at: %s", serverxml), e);
			}
		} else {
			throw new IllegalStateException(String.format("%s doesn't exist.", serverxml));
		}
	}
}
